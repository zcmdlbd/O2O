...
public class ProductServiceImpl implements ProductService {
	...
	@Transactional
	/**
	 * 1.处理缩略图，获取缩略图的相对路径值并赋值给product 
	...
	private void addProductImgList(Product product, List<ImageHolder> productImageHolderList) {
		...
		if (productImgList.size() > 0) {
			try {
				...
				throw new ProductOperationException("创建商品详情图势必" + e.toString());
			}
		}
	}
	...
	@Transactional
	//1.若缩略图参数有值，则处理缩略图
	//若原先存在缩略图则先删除再添加新图，之后获取缩略图的相对路径并赋值给product
	//2.若商品详情图列表有参数值，对商品详情图片记录进行同样的操作
	//3.将tb_product_img下面的该商品的原先的详情图片记录全部删除
	//4.更新tb_product_img以及tb_product信息
